...
class suggestions:
    ...
    async def add(self, bot, ctx, type="general", img=None, embed=None, check=True, *args):
        ...
        if check:
            #print(os.getcwd())
            jsn = json.load(open("suggestions.json"))
            am = str(jsn['count'] + 1)
            jsn['count'] = jsn['count'] + 1
            jsn[am] = {}
            dat = jsn[am]
        ...
        #print(jsn['list'], json.dumps({"h": jsn['list']}))
        ...
        if check:
            if jsn['list'] == None:
                jsn['list'] = []
            jsn['list'].append(msg.id)
            json.dump(jsn, open("suggestions.json", "w"), sort_keys=True, indent=2)
